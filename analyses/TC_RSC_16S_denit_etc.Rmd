...
denit_rates_WINTER <- read.csv("../data/20230131/TownCreek_RSC_DEA_20230131.csv")

# Read in denitrification rates from Town Creek DEA, 2023-03-29 sampling:
denit_rates_SPRING <- read.csv("../data/20230329/TownCreek_RSC_DEA_20230329.csv")

# Read in denitrification rates from Town Creek DEA, 2023-06-21 sampling:
denit_rates_SUMMER <- read.csv("../data/20230621/TownCreek_RSC_DEA_20230621.csv")
...
```{r sediment nitrogen and carbon data}
# Read in 2023 RSC N species concentration data:
# CONSOLIDATE THESE NEXT 3 DFs?
...
# Sediment Total Carbon (TC) and Total Nitrogen (TN) from NCSU EAL (2024-10-14) (look in Desktop/PeraltaLab/Data/NCSU_EAL_CNH2_TC_WRC/Finlay_2024-10-14.xlsx):

# 2015 "chem" data: 
```

```{r import sediment pH data}
sed_pH <- read.csv("../data/pH_TC_RSC.csv", header = T)
sed_pH <- sed_pH[,1:7]
```

# Meta data (design) and wrangling

```{r read in meta data}
# 2015 design file has already been read in as 'design_2015'

#Read in design file for 2023 RSC DEAs
design_2023_RSC_DEA <- read.csv("../data/Design_HC_LD.csv")
```
